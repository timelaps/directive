var STATE = 'state',
    ...
    fromArrayLike = require('@timelaps/to/array/from/array-like'),
    ...
    /**
     * @classdesc Logic class for abstracting logic away from event handlers and having to set any of them up. The Logic object works by distilling all of the values into a binary system of measurement i.e. did you pass the test or did you fail it. If block of logic fails, then the Logic manager can be put into a falsey state. However it is possible to have multiple blocks of logic, as you will see with the or operator that you can use with this class.
     * @class  Logic
     * @example <caption>The Logic class uses a series of logic blocks grouped one layer deep (referred to as logic groups) with or statements in order to achieve a linguistically natural way of abstracting logic behind function calls that appear very similar to sentences.</caption>
     ...
     */
    /**
     * @lends Logic.prototype
     */
    Logic = module.exports = Directive.extend(Logic, {
        /*
         * This function creates the Lingustics object that is automatically added to the LogicManager object that the when method was invoked from.
         * @param {object} origin object that the {@link Logic} class will listen to for events.
         * @returns {this}
         */
        lifecycle: {
        ...
        methods: {
             ...
             * @method
             * @param {Function} handler callback that gets triggered every time the aggregate state of the Logic object changes.
             * @returns {this}
             * @example <caption>an origin initializes with when and sets up one block of logic, and sets up a handler afterward.</caption>
             * origin
             *     .when('bold').is(true)
            ...
            is: addValue(),
            /**
             * isNot is a value associator. Just like [is]{@link Logic#is}, isNot associates a value with the string that was passed in from the when, and, or or call just before this method was called.
             * @method
             * @param {*} comparison if not a function then the object or value will be wrapped in a equality comparison function, namely {@link _.isEqual} and compared when needed to the current value. If it is a function, then it will be responsible for resoving and returning a true or false state.
             * @returns {this} manager call originated from
             * @example
             ...
             * var Logic = origin.when()... // yada yada yada
             * Logic.origin();
             */
            origin: function () {
                return this.directive(REFS).get(INSTANCES, ORIGIN);
            },
            /**
            ...
            and: function (key) {
                var sequencer = this;
                sequencer.directive(REFS).keep(INSTANCES, CURRENT, key);
                return sequencer;
            },
            /**
             ...
             */
            current: function () {
                return this.directive(REFS).get(INSTANCES, CURRENT);
            },
            /**
            ...
            /*
             * Creates the next logic group if any blocks have been created by and, or when statements. Will not need to be used externally. mostly useful for internal ([or]{@link Logic#or}) methods.
             * @return {this}
             */
            group: function () {
                var sequencer = this;
             ...
             * @example
             * var fn = Logic.value(true, function (value) {
             ...
             * Logic.add(5, true, function (static) {
             ...
             * @example
             * var Logic = origin.when("key").is(true)
             ...
             * @return {this}
             * @example
             * var Logic = origin.when("key");
             */
            handle: function (key, arg) {
                var sequencer = this,
                    refs = sequencer.directive(REFS),
                    list = refs.get('collections', key, returnsArray);
                forEach(list, function (bound) {
                    bound(arg);
                });
                return sequencer;
            },
            ...
            /**
             * Applies the correct state to the sequencer and if that state has changed, then it will trigger all of the handlers associated with that sequencer that need to be applied. If an argument is passed, then the change counter will be checked before the [check]{@link Logic#check} method is called.
             * @param  {*} e Argument to pass to all of the handlers. [Run]{@link Logic#run} is called internally to this method.
             * @return {Logic}
...
function push(where) {
    return function (fn) {
        this.directive(REFS).get('collections', where, returnsArray).push(bindTo(fn, this));
        return this;
    };
}
...
function addValue(constant1, constant2) {
    return function () {
        ...
        forEach(fromArrayLike(arguments), function (value) {
